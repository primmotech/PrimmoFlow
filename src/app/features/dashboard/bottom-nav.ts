...
@Component({
  ...
  styles: [`
    .bottom-nav {
      ...
      height: 75px;
      background: var(--card-bg); /* Utilise ta variable CSS */
      border-top: 1px solid var(--border-color); /* Utilise ta variable CSS */
      display: flex;
    ...
    .nav-item {
      ...
      align-items: center;
      color: var(--text-secondary); /* Utilise ta variable CSS */
      gap: 5px;
      flex: 1;
      cursor: pointer;
    ...
    /* Le container pour le bouton + surélevé */
    .add-btn-container {
      flex: 1;
      display: flex;
    ...
    .main-add-btn {
      position: absolute;
      top: -45px; /* Pour le faire sortir vers le haut */
      width: 60px;
      height: 60px;
      border-radius: 30px;
      background: var(--accent-color); /* Bleu PrimmoFlow */
      border: 5px solid var(--card-bg); /* Contour blanc ou noir selon le mode */
